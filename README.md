...
`ruby-2.2.1` | `Time taken for tests:   177.846 seconds`<br/>`Complete requests:      10000`<br/>`Failed requests:        0`<br/>`Write errors:           0`<br/>`Total transferred:      110400000 bytes`<br/>`HTML transferred:       106930000 bytes`<br/>`Requests per second:    56.23 [#/sec] (mean)`<br/>`Time per request:       17.785 [ms] (mean)`<br/>`Time per request:       17.785 [ms] (mean, across all concurrent requests)`<br/>`Transfer rate:          606.21 [Kbytes/sec] received` |đ
 - Cách cài đặt:
